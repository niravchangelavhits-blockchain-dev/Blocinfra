...
const chaincodeController = {
    ...
    async getItemsPaginated(req, res) {
        try {
            ...
            const couchDbUrl = `http://${process.env.COUCHDB_USERNAME || 'admin'}:${process.env.COUCHDB_PASSWORD || 'adminpw'}@${process.env.COUCHDB_HOST || 'localhost'}:${process.env.COUCHDB_PORT || 5984}/${process.env.COUCHDB_DB || 'mychannel_pharma'}/_find`;
